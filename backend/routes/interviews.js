...
router.put('/:id/result', authMiddleware, requireRole('interviewer', 'hr_manager', 'admin'), (req, res) => {
  ...
  const updated = update('interviews', req.params.id, {
    result,
    notes,
    rating,
    status: 'done',
...
// POST /api/interviews/draft-email — generate invite email
router.post('/draft-email', authMiddleware, requireRole('hr_manager', 'admin'), async (req, res) => {
  const { candidateId, interviewDate, interviewerName, jobTitle, branch } = req.body;
  const candidate = findById('candidates', candidateId);
  if (!candidate) return res.status(404).json({ error: 'المرشح غير موجود' });

  const email = await generateInterviewEmail(candidate, interviewDate, interviewerName, jobTitle, branch);
  res.json(email);
});

// POST /api/interviews/questions — AI interview questions
